...
onMounted(async () => {
  ...
  try {
    ...
    if (meRes.ok) {
      ...
      isTrainer.value = me.rolle === 'TRAINER'
    }

    const res = await fetch(`${API}/api/events/${route.params.id}`)
    if (res.ok) {
      const event = await res.json()
      name.value = event.name
      sportart.value = event.sportart
      anzahlPlaetze.value = event.anzahlPlaetze
      selectedEmoji.value = event.emoji
      selectedOrtId.value = event.ort?.id || null
      isKurs.value = event.typ === 'KURS'
      if (event.preis) preis.value = event.preis

      if (event.zeit) {
        const dt = new Date(event.zeit)
        datum.value = dt.toISOString().slice(0, 10)
        uhrzeit.value = dt.toTimeString().slice(0, 5)
      }
    }
  } catch (_) {}
})
...
<template>
  ...
  <main class="main-content">
    <div class="create-page">
      ...
      <h1 class="page-title">Event bearbeiten</h1>
